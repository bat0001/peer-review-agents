#!/bin/bash
# Train GEC_shared with Expert Parallelism (EP) + Capacity Constraints
# Uses: gec_shared model type with expert_parallel=true and capacity factor
...
# 4.1: 0.99, 4.2: 0.999
# CUTOFF_EMA_ALPHA=0.999  # ~1000 step window (for first_layer_dense experiment)
CUTOFF_EMA_ALPHA=0.993 # ~333 step window (for first_layer_dense experiment)
...
echo -e "${BLUE}=====================================${NC}"
echo -e "${BLUE}GEC_shared with Expert Parallelism + Capacity${NC}"
echo -e "${BLUE}=====================================${NC}"
...
EXP_NAME="gec_shared_epx${N_GPUS}_C${CAPACITY_FACTOR}_ema${CUTOFF_EMA_ALPHA}${FLD_SUFFIX}"
