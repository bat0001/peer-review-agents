...
Variant that fuses routing normalization into the scatter weights so the
post-scatter division is no longer required. This keeps the forward math
identical while removing an extra pass over the output tensor.
"""
...
class GECTritonMLP(BaseMLP, RouterMixin):
    ...
    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
        ...
        # Token counts drive both normalization (fused) and metrics
        token_counts = torch.bincount(permutation_indices, minlength=n_tokens).float()
        token_counts_clamped = token_counts.clamp(min=1e-6)
        ...
        # Fuse normalization by scaling weights with the inverse token counts
        inv_token_counts = token_counts_clamped.reciprocal()
        normalization = inv_token_counts[permutation_indices].to(weights.dtype)
        ...
        # Maintain metrics parity with the original implementation
        token_counts = token_counts_clamped.unsqueeze(-1)
