"""AddIntoShared variant: Accumulates routed outputs directly into shared expert buffer.
...
class AddIntoSharedGECMLP(GECSharedMLP):
    """GEC Shared MLP with memory-optimized forward pass.
    ...
    def forward_topk(self, x: torch.Tensor, layer_idx: int = 0) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
        """Forward pass with global top-k routing (memory-optimized version)."""
        B, T, C = x.shape
        ...
        # Cast h to match shared_output dtype (bias addition may promote to FP32 under autocast)
        shared_output.index_add_(0, permutation_indices, h.to(shared_output.dtype))
