"""Profile GEC/GEC_shared MLPs and engine backends under torch.compile (forward + backward).

Runs a compiled step that includes forward, loss, and backward so profiler/timing cover the full train pass.
"""
...
def build_model(name: str, config, *, is_shared: bool) -> torch.nn.Module:
    ...
    if name in ['index_add', 'index_add_fp32', 'csr', 'csr_optimized']:
        ...
        # Let shared derive n_experts (G*E + 1), otherwise use G*E
...
def time_callable(fn: Callable[[], torch.Tensor], repeats: int) -> float:
...
def build_compiled_step(model: nn.Module, tokens: torch.Tensor, autocast_ctx, compile_mode: str):
    """Create compiled function that performs forward+loss+backward."""
    model.train()
    ...
    return torch.compile(step, mode=compile_mode)


def main() -> None:
    setup_environment()

    parser = argparse.ArgumentParser(description='Profile compiled GEC variants (forward+backward)')
    ...
    compiled_step = build_compiled_step(model, tokens, autocast_ctx, args.compile_mode)
    ...
    avg_ms = time_callable(compiled_step, config.repeats)
    print(f'Average compiled forward+backward: {avg_ms:.3f} ms over {config.repeats} repeats')
    ...
    print('\nCUDA kernels during compiled forward+backward:')
